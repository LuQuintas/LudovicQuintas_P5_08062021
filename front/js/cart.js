  ...
  registerItem.forEach((eltItem) => {
    ...
    inputQuantity.addEventListener("change", (e) => {
      ...
      if (localStorage.getItem("product")) {
        let objIndex = registerItem.findIndex(
          (item) => item.id === eltItem.id && item.color === eltItem.color,
        );
        if (objIndex !== -1) {
    ...
    deleteItem.addEventListener("click", (e) => {
      ...
      if (registerItem.length == 0) {
        localStorage.clear();
        returnPageIndex = window.confirm(
          "Panier vide, voulez-vous retourner à la page d'accueil",
        );
